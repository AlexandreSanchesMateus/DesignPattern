...
public class PokemonDatabaseTests
{
    ...
    {
        // Arrange
        DataReader db = new GameObject("test").AddComponent<DataReader>().Init();

        List<Pokemon> tmpdata = db.GetPokemons().ToList();

        // Act
        List<Pokemon> result = db.PokemonByType(type).ToList();
    ...
    public void GetTop5PokemonIdByPowerWithPower()
    {
        ...
        // Act
        List<(int, int)> result = db.DecoratePokemonIdWithPower(db.GetTopPokemonByBasePower(5)).ToList();
